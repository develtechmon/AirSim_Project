...
def train_new_model(timesteps=500_000, save_dir="./airsim_models"):
    ...
    return model, train_env, save_dir
...
def continue_training(model_path, vec_norm_path, additional_timesteps=100_000, save_dir="./airsim_models"):
...
def main():
    ...
    if choice == "1":
        ...
        timesteps = input("  Training timesteps (default 500000): ").strip()
        ...
        print("This will take several hours. Progress saved automatically.")
        ...
        if not vec_norm_path:
            # Try to guess the normalization file
            base_name = os.path.basename(model_path)
            dir_name = os.path.dirname(model_path) or "."
            
            # Try common patterns
            possible_norms = [
                os.path.join(dir_name, f"vec_normalize_{base_name.split('_')[-1]}.pkl"),
                os.path.join(dir_name, "vec_normalize.pkl"),
                model_path.replace("airsim_drone", "vec_normalize").replace(".zip", ".pkl") + ".pkl"
            ]
            
            for norm_path in possible_norms:
                if os.path.exists(norm_path):
                    vec_norm_path = norm_path
                    print(f"  Found normalization file: {vec_norm_path}")
                    break
