"""
DRONE HOVER DISTURBANCE ENVIRONMENT (13 OBSERVATIONS)
======================================================
Stage 2: Adds random wind disturbances to the hover environment.

UPDATED: Now uses 13 observations (includes angular velocity)
This maintains compatibility with Stage 1 and enables transfer to Stage 3!
...
class DroneHoverDisturbanceEnv(gym.Env):
    ...
    def __init__(self, target_altitude=30.0, max_steps=500, wind_strength=5.0, debug=False):
        super().__init__()
        ...
        # Observation space: position + velocity + orientation + angular_velocity
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(13,),  # ← UPDATED from 10 to 13!
            dtype=np.float32
        )
        ...
        if self.debug:
            print(f"✓ Drone Hover Disturbance Environment (13 observations)")
    ...
    def _get_observation(self):
        ...
        obs = np.array([
            ...
            ang_vel.x_val, ang_vel.y_val, ang_vel.z_val  # ← ADDED!
        ], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # Reward calculation (PROVEN from your Stage 2)
        reward = 0
...
if __name__ == "__main__":
    ...
    print("Testing disturbance environment (13 observations)...")
