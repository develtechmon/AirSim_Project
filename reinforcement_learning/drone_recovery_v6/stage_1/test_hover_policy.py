"""
TEST HOVER POLICY (13 OBSERVATIONS)
====================================
Tests the trained neural network policy (not PID expert!) to see if it learned to hover.

UPDATED: Tests with 13 observations (includes angular velocity)
...
python test_hover_policy.py (i'm using this command togenerate 4000 samples)
...
class HoverPolicy(nn.Module):
    """Same architecture as training (13 observations)"""
...
def get_observation(client):
    """Get current observation from drone (13 values)"""
...
def test_policy(model_path, num_episodes=10, max_steps=500, target_alt=30.0):  # ← CHANGED: 10.0 → 30.0
    """Test the learned policy"""
    
    print("\n" + "="*70)
    print("🧪 TESTING LEARNED HOVER POLICY (13 OBSERVATIONS)")
    print("="*70)
    ...
    print("   📊 Observation space: 13 (pos + vel + ori + ang_vel)")
    print()
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps):
            ...
            # ← CHANGED: Altitude bounds updated for 30m
            if alt < 5 or alt > 50:  # Allow range 5-50m (was 2-30m)
    ...
    if success_rate >= 80:
        ...
        print("   ✅ Model can be used for transfer learning!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--altitude', type=float, default=30.0,  # ← CHANGED: 10.0 → 30.0
