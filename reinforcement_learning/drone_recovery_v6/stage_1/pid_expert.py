"""
PID EXPERT CONTROLLER FOR HOVER (13 OBSERVATIONS)
==================================================
...
UPDATED: Now includes angular velocity in observations (13 total)
This allows transfer learning to Stage 3 (flip recovery)
...
Expected Output:
    Mean altitude: ~30.2m (target: 30.0m)
    Std deviation: <0.1m
    Max error: <0.5m
    Status: ✅ EXCELLENT! Ready to generate demonstrations.
...
class PIDExpertHover:
    ...
    UPDATED: Returns 13 observations (includes angular velocity)
    """
    
    def __init__(self, target_altitude=30.0):
        ...
        print("✓ PID Expert Controller Initialized (13 observations)")
        print(f"  Target: Hover at {target_altitude}m")
        print(f"  Control frequency: {1/self.dt:.0f} Hz")
        print(f"  Observation space: 13 (includes angular velocity)")
    ...
    def get_action(self, state):
        ...
        pos = state['position']
        vel = state['velocity']
        # Note: angular_velocity is in state but not used by PID
        # (PID doesn't need it, but neural network will learn from it)
...
def test_pid_expert():
    ...
    print("\n" + "="*70)
    print("🧪 TESTING PID EXPERT CONTROLLER (13 OBSERVATIONS)")
    print("="*70 + "\n")
    ...
    max_error = np.max(np.abs(positions - 30.0))
    
    print("\n" + "="*70)
    print("📊 RESULTS")
    print("="*70)
    print(f"Mean altitude: {mean_alt:.3f}m (target: 30.0m)")
