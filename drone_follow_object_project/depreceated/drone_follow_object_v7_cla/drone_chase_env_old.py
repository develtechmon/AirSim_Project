...
class DroneChaseEnv(gym.Env):
    ...
    def __init__(self, 
                 max_altitude=0,           # At ground level (changed from 50!)
                 min_altitude=-40,         # 40m up (changed from -50)
                 hit_threshold=2.0,
                 max_distance=50,          # MUCH closer (was 100!)
                 spawn_radius=25):         # Closer spawns (was 30)
        ...
        self.max_altitude_diff = 10.0  # NEW: Max altitude difference from target
        
        # Action space: continuous velocities [vx, vy, vz]
        # INCREASED: Allow faster movement for quicker chasing
        self.action_space = spaces.Box(
            low=np.array([-15.0, -15.0, -15.0]),  # Was -5, now -10!
            high=np.array([15.0, 15.0, 15.0]),    # Was 5, now 10!
            dtype=np.float32
        )
        ...
        self.max_episode_steps = 100  # REDUCED from 200! Faster episodes = faster learning
        
        # NEW: Track if drone is making progress
        self.steps_without_progress = 0  # Count steps where distance doesn't decrease
        self.max_steps_without_progress = 5  # VERY strict! (was 10) Force fast movement
    ...
    def step(self, action):
        ...
        # action = [vx, vy, vz] where:
        #   vx = velocity in X axis (lateral: positive = right, negative = left)
        #   vy = velocity in Y axis (forward: positive = forward, negative = backward)
        #   vz = velocity in Z axis (vertical: positive = down, negative = up in NED)
        vx, vy, vz = action
        duration = 1.0  # Execute action for 1.0 second
        
        # Calculate action magnitude - reward for TRYING to move
        action_magnitude = np.sqrt(vx**2 + vy**2 + vz**2)
        
        # Use WORLD FRAME velocity control (not body frame)
        # This means velocities are in global coordinates, not relative to drone orientation
        self.client.moveByVelocityAsync(
            float(vx), float(vy), float(vz), 
            duration=duration,
            drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,
            yaw_mode=airsim.YawMode(False, 0)  # Don't rotate, keep yaw fixed
        ).join()
        ...
        # NEW: Check if altitude difference is too large
        ...
        # Additional penalty for wrong altitude
        ...
        truncated = (
            ...
            altitude_too_different or                     # Altitude too far from target
            is_inactive or                                # Drone is inactive/stuck
            self.episode_steps >= self.max_episode_steps  # Max steps reached
        )
        
        # DEBUG PRINTS - so you can see what's happening!
        ...
        if hit_target:
            print(f"\n🎯 Target HIT at step {self.episode_steps}! Respawning...")
            self._spawn_target()
            self.previous_distance = self._calculate_distance(drone_pos, self.target_position)
            self.steps_without_progress = 0  # Reset inactivity counter
    ...
    def _calculate_reward(self, current_distance, previous_distance, 
                          ...
                          action_magnitude):
        """
        SIMPLIFIED AND AGGRESSIVE reward function
        Every component is printed for debugging
        """
        reward = 0.0
        reward_breakdown = []
        
        # ═══════════════════════════════════════════════════════════
        # 1. HIT TARGET - JACKPOT!
        # ═══════════════════════════════════════════════════════════
        if hit_target:
            reward += 1000.0  # HUGE reward!
            reward_breakdown.append(f"HIT_TARGET:+1000.0")
            print(f"  💰 Reward breakdown: {' | '.join(reward_breakdown)} = {reward:.1f}")
            return reward
        
        # ═══════════════════════════════════════════════════════════
        # 2. OUT OF BOUNDS - DISASTER!
        # ═══════════════════════════════════════════════════════════
        if out_of_bounds:
        ...
        # ═══════════════════════════════════════════════════════════
        # 3. DISTANCE CHANGE - The most important signal!
        # ═══════════════════════════════════════════════════════════
        distance_delta = previous_distance - current_distance
        
        if distance_delta > 0.5:  # Got significantly closer
            distance_reward = distance_delta * 50.0  # MASSIVE reward
            reward += distance_reward
            reward_breakdown.append(f"CLOSER:+{distance_reward:.1f}")
        
        elif distance_delta < -0.5:  # Moved significantly away
            distance_penalty = abs(distance_delta) * 100.0  # CRUSHING penalty
            reward -= distance_penalty
            reward_breakdown.append(f"AWAY:-{distance_penalty:.1f}")
        
        else:  # No significant change (wobbling or hovering)
            hover_penalty = 100.0
            reward -= hover_penalty
            reward_breakdown.append(f"NO_PROGRESS:-{hover_penalty:.1f}")
        
        # ═══════════════════════════════════════════════════════════
        # 4. ACTION MAGNITUDE - Reward for trying + SMOOTHNESS
        # ═══════════════════════════════════════════════════════════
        if action_magnitude > 3.0:  # Strong, fast action
            action_reward = action_magnitude * 10.0  # BIG reward for fast movement!
            reward += action_reward
            reward_breakdown.append(f"FAST_ACTION:+{action_reward:.1f}")
        elif action_magnitude > 2.0:  # Decent action
            action_reward = action_magnitude * 5.0
            reward += action_reward
            reward_breakdown.append(f"STRONG_ACTION:+{action_reward:.1f}")
        elif action_magnitude < 0.5:  # Weak action (wobbling)
            action_penalty = 50.0
            reward -= action_penalty
            reward_breakdown.append(f"WEAK_ACTION:-{action_penalty:.1f}")
        
        # NEW: Reward for moving in the RIGHT DIRECTION (toward target)
        # Calculate if action is aligned with target direction
        if hasattr(self, 'target_position') and current_distance > 0:
            ...
            if target_dir_mag > 0:
                dx_norm = dx_target / target_dir_mag
                dy_norm = dy_target / target_dir_mag
                dz_norm = dz_target / target_dir_mag
                
                # Get action from previous step (we need to store this)
                # For now, estimate from distance change direction
                # Positive alignment = moving in right direction
                # This will be more accurate with stored action
                
                # Simplified: If distance is decreasing AND action is strong = good alignment
                if distance_delta > 0 and action_magnitude > 2.0:
                    alignment_reward = 20.0
                    reward += alignment_reward
                    reward_breakdown.append(f"ALIGNED:+{alignment_reward:.1f}")
        
        # ═══════════════════════════════════════════════════════════
        # 5. ALTITUDE MATCHING - Should be at same height as target
        # ═══════════════════════════════════════════════════════════
        if altitude_diff > 5.0:
            altitude_penalty = altitude_diff * 5.0
            reward -= altitude_penalty
            reward_breakdown.append(f"ALTITUDE:-{altitude_penalty:.1f}")
        
        # ═══════════════════════════════════════════════════════════
        # 6. DISTANCE PENALTY - Being far is bad
        # ═══════════════════════════════════════════════════════════
        ...
        # ═══════════════════════════════════════════════════════════
        # 7. TIME PENALTY - Encourages speed
        # ═══════════════════════════════════════════════════════════
        time_penalty = 5.0
        reward -= time_penalty
