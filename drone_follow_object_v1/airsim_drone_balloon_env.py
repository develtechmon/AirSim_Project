...
class AirSimDroneBalloonEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # Think of it as: X=forward/back, Y=right/left, Z=down/up
        self.boundary_min = np.array([-50.0, -50.0, -50.0])  # min x, y, z
        self.boundary_max = np.array([50.0, 50.0, -5.0])     # max x, y, z
    ...
    def reset(self, seed=None, options=None):
        ...
        start_x = self.np_random.uniform(-20, 20)
        start_y = self.np_random.uniform(-20, 20)
        start_z = -10.0  # 10 meters up
    ...
    def step(self, action):
        ...
        if action == 1:  # forward (positive X)
            velocity[0] = self.drone_speed
        elif action == 2:  # backward (negative X)
            velocity[0] = -self.drone_speed
        elif action == 3:  # left (negative Y)
            velocity[1] = -self.drone_speed
        elif action == 4:  # right (positive Y)
            velocity[1] = self.drone_speed
        ...
        if out_of_bounds:
            ...
            if distance < self.hit_distance:
                # Balloon popped! Big reward (same as 2D)
                reward = 10.0
                self.score += 1
                
                # Spawn new target if not reached max score
                if self.score < self.max_score:
                    self.target_pos = self._spawn_target()
                    self._create_target_visual()
            else:
                # SAME REWARD STRUCTURE AS YOUR 2D CODE
                # Small penalty for each step + reward for getting closer
                prev_distance = np.linalg.norm(previous_pos - self.target_pos)
    ...
    def _spawn_target(self):
        """Spawn target at random position away from drone"""
        while True:
            # Random position within boundaries
            pos = np.array([
                self.np_random.uniform(self.boundary_min[0] + 10, self.boundary_max[0] - 10),
                self.np_random.uniform(self.boundary_min[1] + 10, self.boundary_max[1] - 10),
                self.np_random.uniform(self.boundary_min[2] + 5, self.boundary_max[2] - 5)
            ], dtype=np.float32)
            
            # Ensure target spawns at least 15 meters away from drone
