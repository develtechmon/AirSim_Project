...
class DroneChaseEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.max_altitude_diff = 15.0
        
        # Action space: continuous velocities [vx, vy, vz]
        # INCREASED for more aggressive movement
        self.action_space = spaces.Box(
            low=np.array([-15.0, -15.0, -15.0]),
            high=np.array([15.0, 15.0, 15.0]),
            dtype=np.float32
        )
        
        # Observation space: [dx, dy, dz, vx, vy, vz, distance, altitude_diff]
        self.observation_space = spaces.Box(
            low=np.array([-200, -200, -200, -15, -15, -15, 0, -100]),
            high=np.array([200, 200, 200, 15, 15, 15, 200, 100]),
            dtype=np.float32
        )
        ...
        self.max_steps_without_progress = 3
    ...
    def reset(self, seed=None, options=None):
        ...
        time.sleep(0.1)  # Give AirSim time to clear
    ...
    def step(self, action):
        ...
        # HARSHER penalty for being inactive
        if is_inactive:
            reward -= 100.0
        ...
        if hit_target:
            ...
            self.client.simFlushPersistentMarkers()
            time.sleep(0.15)  # Increased wait time for proper clearing
    ...
    def _calculate_reward(self, current_distance, previous_distance, 
                          ...
                          action_magnitude):
        """
        AGGRESSIVE reward function - heavily rewards FAST movement toward target
        """
        reward = 0.0
        reward_breakdown = []
        
        # Hit target - MASSIVE JACKPOT!
        if hit_target:
            reward += 2000.0
        ...
        # Out of bounds - CRUSHING PENALTY!
        if out_of_bounds:
            reward -= 1000.0
            reward_breakdown.append(f"OUT_OF_BOUNDS:-1000.0")
            print(f"  💰 Reward breakdown: {' | '.join(reward_breakdown)} = {reward:.1f}")
            return reward
        ...
        if distance_delta > 0.3:
            distance_reward = distance_delta * 100.0
            reward += distance_reward
            reward_breakdown.append(f"CLOSER:+{distance_reward:.1f}")
        
        elif distance_delta < -0.3:
            distance_penalty = abs(distance_delta) * 200.0
        ...
        # Action magnitude
        if action_magnitude > 8.0:
            action_reward = action_magnitude * 20.0
            reward += action_reward
            reward_breakdown.append(f"SUPER_FAST:+{action_reward:.1f}")
        elif action_magnitude > 5.0:
            action_reward = action_magnitude * 15.0
            reward += action_reward
            reward_breakdown.append(f"FAST_ACTION:+{action_reward:.1f}")
        elif action_magnitude > 3.0:
            action_reward = action_magnitude * 8.0
            reward += action_reward
            reward_breakdown.append(f"STRONG_ACTION:+{action_reward:.1f}")
        elif action_magnitude < 1.0:
            action_penalty = 100.0
    ...
    def _spawn_target(self):
        ...
        self.client.simFlushPersistentMarkers()
        time.sleep(0.15)  # Give AirSim time to clear properly
